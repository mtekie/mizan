...
import { allProducts as products, getProductsByCategory, type ProductCategory } from '@/lib/data/products';
...
export default function Catalogue() {
  ...
  const filteredProducts = getProductsByCategory(activeFilter).filter((p) => {
    // 1. Filter by Product Type (Loan vs Savings)
    ...
    const bankMatch = (p.bankName || p.instituteId || '').toLowerCase().includes(searchQuery.toLowerCase());
    return titleMatch || bankMatch;
  });

  // Get top featured products across all categories
  const featuredProducts = [...products]
    .filter(p => p.highlight || (p.matchScore && p.matchScore > 85))
  ...
  categories.forEach(c => {
    categoryCounts[c.key] = c.key === 'All' ? products.length : products.filter(p => p.category === c.key).length;
  });

  // Unique institutions from filtered results
  const institutions = [...new Set(filteredProducts.map(p => p.bankId || p.instituteId))].filter(Boolean);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-slate-500">{products.length} products across {banks.length} institutions</p>
            </div>
          </div>
          {/* Search */}
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-black text-slate-900">
                  ...
                  <span className="text-sm font-normal text-slate-400 ml-2">({filteredProducts.length})</span>
                </h2>
              </div>
            ...
            <div className="sticky top-28">
              ...
              <div className="space-y-2 max-h-[calc(100vh-200px)] overflow-y-auto hide-scrollbar">
                {institutions.slice(0, 20).map(bankId => {
                  ...
                  const bankProds = filteredProducts.filter(p => (p.bankId === bankId || p.instituteId === bankId));
                  ...
                  return (
                    <Link href={`/catalogue/bank/${bankId}`} key={bankId} className="block group">
                      <div className="bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-3 hover:shadow-sm transition-all">
                        <div className={`w-10 h-10 rounded-lg ${bank?.color || rep?.bankIconBg || 'bg-slate-200'} ${bank?.textColor || 'text-white'} flex items-center justify-center font-bold text-xs shrink-0`}>
                          {bank?.logo || rep?.bankLogo || 'FI'}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="text-sm font-semibold text-slate-900 truncate group-hover:text-[#3EA63B] transition-colors">
                            {bank?.name || rep?.bankName || bankId}
                          </h4>
                          <p className="text-[10px] text-slate-400 font-semibold">
                            {bankProds.length} product{bankProds.length > 1 ? 's' : ''}
                            {bank?.type && bank.type !== 'Commercial Bank' && ` · ${bank.type}`}
                          </p>
                        </div>
