...
// MOCK data — replace with real cashflow API in production
const cashFlowData = [
...
export default function DashboardClient({ user, accounts, transactions, summary, featuredProducts }: { user: any, accounts: any[], transactions: any[], summary: any, featuredProducts: any[] }) {
  ...
  // ── Simple Mode: Mint-inspired dashboard (Gated to mobile only) ──
  if (uiMode === 'simple' && isMobile) {
    return <SimpleDashboard user={user} accounts={accounts} transactions={transactions} summary={summary} />;
  }

  // ── Pro Mode: Full 12-column dashboard ──
  return (
      ...
      <header className="md:hidden pt-12 pb-4 px-6 flex justify-between items-center bg-white sticky top-0 z-20 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-lg font-bold text-[#0F172A]">{user?.name?.split(' ')[0] || 'User'} <span className="font-normal text-sm font-ethiopic text-slate-400">👋</span></h2>
          </div>
        </div>
      ...
      <div className="flex-1 p-4 md:p-6 max-w-[1600px] mx-auto w-full flex flex-col gap-4 pb-24 md:pb-6">
        ...
        <div className="hidden md:flex flex-col gap-4">
          <div className="flex justify-between items-end">
            <div className="flex items-baseline gap-3">
              <h1 className="text-2xl font-black text-[#0F172A] tracking-tight">{user?.name?.split(' ')[0] || 'User'} <span className="text-slate-400 font-normal">👋</span></h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm flex flex-col justify-center">
              ...
              <div className="flex items-end gap-2">
                <span className="text-xl font-black text-[#0F172A] leading-none">{summary.netWorth.toLocaleString()}</span>
            ...
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm flex flex-col justify-center">
              ...
              <div className="flex items-end gap-2">
                <span className="text-lg font-bold text-[#3EA63B] leading-none">+{summary.monthlyIn.toLocaleString()}</span>
                <span className="text-[10px] font-bold text-slate-400 mb-0.5">ETB</span>
              </div>
            </div>
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm flex flex-col justify-center">
              ...
              <div className="flex items-end gap-2">
                <span className="text-lg font-bold text-amber-600 leading-none">-{summary.monthlyOut.toLocaleString()}</span>
                <span className="text-[10px] font-bold text-slate-400 mb-0.5">ETB</span>
              </div>
            </div>
            <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3">
                <span className="text-lg font-black text-[#0F172A] leading-none">{summary.savingsRate}%</span>
                <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="w-[${summary.savingsRate}%] h-full bg-[#3EA63B]" style={{ width: `${summary.savingsRate}%` }}></div></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-3 flex flex-col gap-4">
            ...
            <Link href="/score" className="bg-[#0F172A] rounded-xl p-4 shadow-sm group hover:ring-2 ring-[#3EA63B] transition-all relative overflow-hidden">
              ...
              <div className="flex items-end gap-3 relative z-10">
                <span className="text-4xl font-black text-white leading-none">{user?.score || 600}</span>
                <div className="pb-1">
                  <span className="text-[10px] text-[#6ED063] font-bold flex items-center gap-0.5"><Activity className="w-3 h-3" /> Dashboard</span>
                </div>
              </div>
            </Link>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex-1 flex flex-col relative overflow-hidden">
              ...
              {accounts.length > 0 ? (
                <div className="flex-1 relative mt-2 group perspective-1000 min-h-[150px]">
                    {accounts.slice(0,4).map((asset, i) => (
                    <div
                        key={asset.id}
                        ...
                        style={{
                        backgroundColor: asset.color || '#0F172A',
                        top: `${i * 35}px`,
                        zIndex: i * 10,
                        transform: `scale(${1 - (Math.min(accounts.length, 4) - 1 - i) * 0.05}) translateZ(0)`,
                        opacity: 1 - (Math.min(accounts.length, 4) - 1 - i) * 0.05
                        }}
                    >
                        ...
                        {!['Bank Book', 'Debit Card', 'Credit Card', 'Mobile Wallet'].includes(asset.type) && <Wallet className="w-5 h-5 text-white/30" />}
                        </div>
                        <div>
                        {asset.type === 'Credit Card' ? (
                            <div className="flex items-baseline gap-2 mb-1">
                            <span className="text-xl font-black drop-shadow-md">
                                {asset.balance.toLocaleString()} <span className="text-[10px] font-bold ml-1 text-white/70">ETB</span>
                            </span>
                            </div>
                        ) : (
                            <div className="mb-1">
                ...
                <div className="flex-1 flex items-center justify-center pt-8 pb-4">
                    <p className="text-xs text-slate-400 text-center">No accounts added yet.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col gap-4">
            {/* Cash Flow Chart - Keeping mock chart array for now until historical data API built */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 relative overflow-hidden group">
                ...
                <div className="flex items-end justify-between mb-2">
                  <div>
                    <span className="text-xl font-black text-slate-900 leading-none">{summary.monthlyOut.toLocaleString()}</span>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-bold text-slate-900 leading-tight">{summary.topGoal?.name || 'Create a Goal'}</h4>
                    <p className="text-[10px] text-slate-400 font-bold">{summary.topGoal ? `${summary.topGoal.saved.toLocaleString()} / ${summary.topGoal.target.toLocaleString()} saved` : 'Start saving today'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${summary.topGoal ? (summary.topGoal.saved / summary.topGoal.target * 100) : 0}%` }}></div>
                  </div>
                  <span className="text-[10px] font-black text-slate-900">{summary.topGoal ? Math.round(summary.topGoal.saved / summary.topGoal.target * 100) : 0}%</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex-1">
              ...
              <div className="space-y-3">
                {transactions.length === 0 ? (
                  <p className="text-xs text-slate-400 pt-4">No transactions yet.</p>
              ...
              <div className="relative z-10">
                ...
                {featuredProducts?.map(p => (
                  <Link href={`/catalogue/${p.id}`} key={p.id} className="block bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg p-2.5 mb-2 last:mb-0 transition-colors">
                    ...
                    <div className="flex justify-between items-end">
                      <span className="text-[10px] font-black bg-[#3EA63B] text-white px-1.5 py-0.5 rounded">{p.matchScore || 90}% Match</span>
                      <Play className="w-3 h-3 text-white/50" />
                    </div>
                  </Link>
                ))}
                {(!featuredProducts || featuredProducts.length === 0) && (
                   <p className="text-[10px] text-indigo-300 italic">Exploring new items for you...</p>
                )}
              </div>
            </div>
