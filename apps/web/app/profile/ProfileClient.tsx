...
export default function ProfileClient({ user, accounts }: { user: any, accounts: any[] }) {
    ...
    // Map db models to the UI format, falling back to mock if empty
    const displayAccounts = accounts.length > 0 ? accounts.map((a: any) => ({
    ...
    const totalBalance = displayAccounts.reduce((s: any, a: any) => s + a.value, 0);
