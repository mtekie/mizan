...
export default function NotificationsClient({ initialNotifications }: { initialNotifications: any[] }) {
  ...
  const notifications = initialNotifications.length > 0 ? initialNotifications.map((n: any) => ({
    id: n.id,
    type: n.type === 'actionable' ? 'Actionable' : 'Info',
    date: new Date(n.createdAt).toDateString() === new Date().toDateString() ? 'Today' : 'Earlier',
    color: n.type === 'actionable' ? 'bg-orange-500' : 'bg-blue-500',
    icon: n.icon === 'Wallet' ? Wallet : n.icon === 'AlertTriangle' ? AlertTriangle : n.icon === 'Trophy' ? Trophy : n.icon === 'TrendingUp' ? TrendingUp : AlertTriangle,
    iconBg: n.type === 'actionable' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600',
    title: n.title,
    titleAmh: n.titleAmh,
    subtitle: n.subtitle,
    time: new Date(n.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
    actions: n.type === 'actionable' ? [{ label: 'Dismiss', style: '' }, { label: 'Review', style: 'text-[#3EA63B]' }] : [],
    faded: n.isRead,
  })) : mockNotifications;

  const filtered = notifications.filter((n: any) => tab === 'All' || n.type === tab);
  const grouped = filtered.reduce((acc: any, n: any) => {
