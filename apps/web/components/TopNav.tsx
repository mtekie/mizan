...
export function TopNav() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            if (user) {
                // You might also fetch from /api/v1/settings for a more updated internal user record
                // For speed, just parsing user metadata or defaulting to email
            ...
            <div className="flex items-center gap-4 ml-8 pl-6 border-l border-slate-800">
                ...
                <Link href="/profile" className="flex items-center gap-2 hover:bg-white/5 py-1 px-2 rounded-lg transition-colors cursor-pointer">
                    <div className="text-right hidden lg:block">
                        <p className="text-xs font-bold text-white leading-tight">{userName.split(' ')[0]}</p>
